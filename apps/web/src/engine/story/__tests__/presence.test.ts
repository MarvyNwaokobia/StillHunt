...
/** The mission beats the StillHunt doorkicker operation fires (the plan slice 6). */
const MISSION_TRIGGERS: PresenceTrigger[] = [
  'opStart', 'opBreach', 'troopsCleared', 'huntFirstWord',
...
const REUSED_RECORDED = ['ember-low-hp', 'ember-troops-cleared', 'ember-zone-clear'];
...
describe('presence lines', () => {
  ...
  it('ends the operation on the antagonist, then Ember naming him (the gate)', () => {
    expect(linesFor('zoneClear')[0].speaker).toBe('hollow');
    const tag = linesFor('zoneClearTag')[0];
    expect(tag.speaker).toBe('ember');
    expect(tag.text).toContain('Hollow');
  });
  ...
  it('the new doorkicker lines never mention the blade the hero no longer carries', () => {
    const newIds = ['ember-op-start', 'ember-op-breach', 'hollow-first-word', 'hollow-push-in', 'hollow-op-down'];

/**
 * The three voices (the antagonist as presence).
 *
 * The gate for this slice: a first-time player can say who they are, who
 * Ember is, and why Hollow scares them, WITHOUT reading a menu. So the story
 ...
 *   STILLHUNT — the name every enemy dies protecting. He has your radio channel.
...
export type Speaker = 'ember' | 'hollow' | 'cinder';
...
export type PresenceTrigger =
  ...
  // ── doorkicker beats (the plan slice 6). The melee triggers above name a
  ...
  | 'huntFirstWord' // he answers the channel Ember just noticed
  | 'opPushIn'       // entering the objective room
  | 'opHeroDown'     // the antagonist over your body (rifle-appropriate)
  | 'missionCleared' // a non-finale op is done (Ember)
  | 'hollowReveal'    // the finale: the antagonist, finally in the room with you
...
export const SPEAKER_META: Record<Speaker, { name: string; color: string }> = {
  ...
  hollow: { name: 'HOLLOW', color: '#e0455a' },
...
export const PRESENCE_LINES: PresenceLine[] = [
  ...
  },
  {
    ...
    text: 'That voice. Ash... that was him. That was Hollow.',
  },

  // ── doorkicker lines (slice 6). No mp3 yet → subtitle + radio static. ──
  {
    id: 'ember-op-start',
    speaker: 'ember',
    trigger: 'opStart',
  ...
  {
    id: 'hollow-first-word',
    speaker: 'hollow',
    trigger: 'huntFirstWord',

 ...
 * @description The tactical audio director for the StillHunt clone (the plan slice 2).
 *
...
export class FpsAudio {
   ...
   */
  vo(id: string, speaker: 'ember' | 'hollow' | 'cinder'): void {
  ...
  private playVoFile(id: string): void {
    try {
      ...
      el.volume = 0.95;
      // StillHunt speaks slowly: hold the subtitle for the RECORDING, not a guess.
  ...
  private radioSignature(speaker: 'ember' | 'hollow' | 'cinder'): void {
    if (speaker === 'ember') {        // close, warm, a friend's channel
      this.noiseBand(1900, 0.1, 0.14);
      this.noiseBand(1400, 0.07, 0.09);
    } else if (speaker === 'hollow') { // low — the channel drops to make room for him

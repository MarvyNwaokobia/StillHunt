 ...
 * head/body hitboxes, cover that eats rounds, lethal StillHunt damage, static dummies
 * that fall and respawn so the feel can be hammered on.
 *
 * Isolated on purpose (Marvy's rule): this mounts at /dev/verb and touches
 * nothing in the live /fight game. It becomes /fight only when the whole StillHunt
 * build is done.
 *
...
const SENS_KEY = 'stillhunt_look_sens';
...
const WEAPON_VIEW: Record<GunId, { scale: number; z: number; y: number }> = {
  ...
  legendary: { scale: 1.0, z: 0, y: 0 },      // the StillHunt Prototype
...
const XP_KEY = 'stillhunt_career_xp';
const XP_POPS = 4;
...
function FpsWorld({ hud, controls, audio, lowSpec, lightFx, minimal, mission, onComplete, onDeath, pausedRef, gateRef, accountRank, accountXp, equippedGun, equippedAmmo, equippedMods, fieldKit, endlessOpts }: {
  ...
  useEffect(() => {
    ...
    w.__huntState = () => sim.snapshot();
    w.__huntProps = () => dressingFor(mission); // set-dressing placements (headless visual check)
    w.__huntKillAll = () => sim.debugKillAll();
    ...
    w.__huntKillRoom = () => sim.debugKillRoom(chain.current.cursor + 1);
    w.__huntKillPlayer = () => sim.debugKillPlayer();
    w.__huntAim = () => ({ pitch: pitch.current, yaw: yaw.current });
    w.__huntAudio = () => audio.stats();
    w.__huntMission = () => ({ objective: objective.current, total: OBJECTIVES.length, complete: completeAt.current > 0, briefing: sim.time < briefingUntil.current, survival, gauntlet: !!mission.gauntlet, wave: survWave.current, waveState: survState.current, survOver: survOver.current, kills: sim.snapshot().stats.kills });
    ...
    w.__huntStory = () => ({ fired: [...storyFired.current], queued: voQueue.current.length });
    w.__huntRig = () => rigApis.current[0]?.debug() ?? null;
    w.__huntPlayer = () => ({ x: +pos.current.x.toFixed(3), z: +pos.current.z.toFixed(3) });
    w.__huntStagger = () => ({
    ...
    w.__huntColliders = () => COLLIDERS;
    w.__huntCam = () => ({ x: +camera.position.x.toFixed(3), z: +camera.position.z.toFixed(3) });
    // audition any presence line (also lets us verify which ids have a recording)
    w.__huntVo = (id: string, speaker: 'ember' | 'hollow' | 'cinder') => audio.vo(id, speaker);
    w.__huntSetXp = (n: number) => { careerXp.current = n; try { window.localStorage.setItem(XP_KEY, String(n)); } catch { /* ignore */ } };
    // headless: wound the live boss to a fraction of its health, to drive phases
    w.__huntHurtBoss = (frac: number) => {
    ...
    w.__huntWakeRoom = (room: number) => sim.setRoomActive(room, true); // headless room activation
    w.__huntSwitch = (n?: number) => (typeof n === 'number' ? sim.switchGun(n) : sim.nextWeapon());
    w.__huntFire = (on: boolean) => { controls.current.fire = !!on; }; // headless: hold the trigger (aim-assist test)
    w.__huntToggle = (a: Attachment) => sim.toggleAttachment(a);
    ...
    w.__huntResupply = () => { if (!survival || !sim.snapshot().playerAlive) return false; sim.resupply(); return true; };
    w.__huntWaveSkip = () => {
      if (!survival || !sim.snapshot().playerAlive) return false;
    ...
    return () => {
      delete w.__huntState; delete w.__huntKillAll; delete w.__huntKillRoom; delete w.__huntKillPlayer; delete w.__huntAim; delete w.__huntAudio;
      delete w.__huntMission; delete w.__huntEndless; delete w.__huntWarp; delete w.__huntSkipBriefing;
      delete w.__huntXp; delete w.__huntSetXp; delete w.__huntStory; delete w.__huntVo; delete w.__huntRig; delete w.__huntPlayer; delete w.__huntColliders; delete w.__huntCam; delete w.__huntStagger; delete w.__huntHurtBoss; delete w.__huntWakeRoom; delete w.__huntSwitch; delete w.__huntFire; delete w.__huntToggle;
      delete w.__huntRevive; delete w.__huntResupply; delete w.__huntWaveSkip; delete w.__huntPerf;
    };
  }, [sim, audio]);
  ...
  useFrame((frame, rawDt) => {
    ...
    for (const ev of sim.drain()) {
      if (ev.kind === 'fire') {
        ...
        // The boss just escalated: pulse it, jolt the view, and let StillHunt mark it.
        bossFlash.current = 1;
    ...
    if (obj && snap.playerAlive && completeAt.current < 0 && now > briefingUntil.current) {
      ...
      if (done) {
        ...
        if (objective.current === 0) say('opBreach');
          ...
          say('troopsCleared');
          // StillHunt first answers the channel ONCE in the whole campaign.
          try {
            if (!window.localStorage.getItem('stillhunt_heard')) { say('huntFirstWord'); window.localStorage.setItem('stillhunt_heard', '1'); }
          } catch { say('huntFirstWord'); }
        } else if (objective.current === 2 && obj.kind === 'clear') {
          say(isFinale ? 'hollowReveal' : 'opPushIn'); // the finale: he's finally in the room
        }
        objective.current++;
  ...
  function pumpStory(now: number) {
    ...
    // the text-length guess cuts StillHunt off (he speaks slowly).
  ...
  return (
    <>
      ...
      {/* ── bodycam post ── C3: the LIGHT stack drops N8AO (a whole normal re-render
          ...
          PerformanceMonitor → `degraded` in HuntScene). Capable machines keep the full look. */}
    ...
    >
      <div style={{ maxWidth: 760, margin: '0 auto' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 22 }}>
          <div>
            <div style={{ fontSize: 12, letterSpacing: 8, color: '#37d0e0' }}>StillHunt · CAMPAIGN</div>
      ...
      <div style={{ maxWidth: 620, textAlign: 'center' }}>
        ...
        {mode === 'finale' ? (
          <>
            ...
            <div style={{ fontSize: 48, fontWeight: 800, letterSpacing: 5, margin: '10px 0', color: '#ff5a47' }}>STILLHUNT IS DEAD</div>
...
function SurvivalRearmControls({ walletAddress }: { walletAddress: string }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      const m = (window as unknown as { __huntMission?: () => { survival?: boolean; gauntlet?: boolean; wave?: number; survOver?: boolean } }).__huntMission?.();
  ...
  const bridge = window as unknown as { __huntRevive?: () => boolean; __huntResupply?: () => boolean; __huntWaveSkip?: () => boolean };
...
function GauntletRunController({ walletAddress }: { walletAddress: string }) {
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      const m = (window as unknown as { __huntMission?: () => { gauntlet?: boolean; wave?: number; survOver?: boolean; kills?: number } }).__huntMission?.();
...
export function HuntScene({ onOpStart, onOpCleared, onOpFailed, startMission, resumeLevel, walletAddress, accountRank, accountXp, equippedGun, equippedAmmo, equippedMods, fieldKit, onExit, endless, seasonal }: {
  ...
  useEffect(() => {
    const w = window as unknown as Record<string, unknown>;
    w.__huntCampaign = () => ({ index: missionIndex, total: CAMPAIGN.length, name: mission.name, zone: mission.zone, done: campaignDone, progress, selectOpen });
    w.__huntNextMission = () => advance();
    w.__huntResetCampaign = () => {
    ...
    w.__huntOpenSelect = (v = true) => setSelect(!!v);
    w.__huntPickMission = (i: number) => pickMission(i);
    w.__huntSurvival = () => pickSurvival();
    w.__huntGauntlet = () => { setMode('gauntlet'); setCampaignDone(false); setDebrief(null); setSelect(false); setRunNonce((n) => n + 1); }; // probe: force ranked mode (bypasses unlock)
    w.__huntPause = (v = true) => (v ? openPause() : resume()); // C4 probe
    w.__huntProgress = () => progress;
    return () => {
      delete w.__huntCampaign; delete w.__huntNextMission; delete w.__huntResetCampaign;
      delete w.__huntOpenSelect; delete w.__huntPickMission; delete w.__huntSurvival; delete w.__huntGauntlet; delete w.__huntPause; delete w.__huntProgress;
    };
  ...
  const [quality, setQuality] = useState<'auto' | 'high' | 'low'>(() => {
    ...
    const q = window.localStorage.getItem('stillhunt_quality');
  ...
  const pickQuality = useCallback((q: 'auto' | 'high' | 'low') => {
    ...
    try {
      window.localStorage.setItem('stillhunt_quality', q);
      // HIGH means "this machine is fine" → forget the auto-detect so it starts full.
      if (q === 'high') window.localStorage.removeItem('hunt_degraded');
    } catch { /* private mode */ }
  }, []);
  ...
  const [degraded, setDegraded] = useState(() => {
    if (typeof window === 'undefined') return false;
    return window.localStorage.getItem('hunt_degraded') === '1';
  });
  const markDegraded = useCallback(() => {
    setDegraded(true);
    try { window.localStorage.setItem('hunt_degraded', '1'); } catch { /* private mode */ }
  }, []);
  ...
  // STILLHUNT is a landscape game. On a phone held upright, block play with a rotate
    ...
    <div ref={(r) => { hud.current.root = r; }} style={{ position: 'fixed', inset: 0, background: '#000', cursor: 'none', touchAction: 'none', userSelect: 'none', WebkitUserSelect: 'none', WebkitTouchCallout: 'none', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', fontFamily: UI_FONT, color: '#e9edf2', userSelect: 'none' }}>
        ...
        <div ref={(r) => { hud.current.bossWrap = r; }} style={{ position: 'absolute', left: '50%', top: 96, transform: 'translateX(-50%)', width: 360, opacity: 0, transition: 'opacity .3s', textAlign: 'center' }}>
          <div ref={(r) => { hud.current.bossName = r; }} style={{ fontSize: 13, letterSpacing: 4, fontWeight: 700, color: '#e0455a', textShadow: '0 0 6px #000', marginBottom: 4 }}>STILLHUNT</div>
        ...
        <div style={{ position: 'absolute', left: isTouch ? 90 : 150, top: 20 }}>
          <span style={{ fontSize: 12, letterSpacing: 2, color: '#6f7d8c' }}>{mode === 'endless' ? `StillHunt · ${seasonal ? 'SEASONAL CAMPAIGN' : 'ENDLESS'} · ${mission.name}` : mode === 'gauntlet' ? 'StillHunt · GAUNTLET · RANKED' : mode === 'survival' ? 'StillHunt · SURVIVAL · THE KILL-HOUSE' : `StillHunt · ${mission.zone} · OP ${missionIndex + 1}/${CAMPAIGN.length}`}</span>
        </div>
        {!isTouch && (
      ...
      {/* ── Landscape gate: a phone held upright can't play STILLHUNT ── */}
      {isTouch && portrait && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 60, background: 'radial-gradient(circle at 50% 40%, #0b1018, #05070b)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#cfe0ea', textAlign: 'center', fontFamily: UI_FONT, pointerEvents: 'auto' }}>
          ...
          <div style={{ fontSize: 13, color: '#7f8c99', letterSpacing: 3, marginTop: 10 }}>STILLHUNT IS PLAYED IN LANDSCAPE</div>
